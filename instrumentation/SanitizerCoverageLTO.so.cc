...
class ModuleSanitizerCoverageLTO
 ...
 public:
  ...
  std::unordered_map<const BasicBlock *, uint32_t> instrument_point_map;
...
bool ModuleSanitizerCoverageLTO::instrumentModule(
  ...
  */
  ...
  const char *out_path = std::getenv("OUT");
  if (!out_path) FATAL("environment OUT not set");

  std::string instrument_num = std::string(out_path) + "/output.txt";
  std::string readable_map = std::string(out_path) + "/output1.txt";
  std::string inst_map = std::string(out_path) + "/instrument_point.bin";

  outFile.open(instrument_num);
  outFile1.open(readable_map);
  out_file.open(inst_map, std::ios::binary);

  if (!outFile.is_open() || !outFile1.is_open() || !out_file.is_open()) {
...
std::string getSourceFilePath(const llvm::Function &F) {

  if (auto *SP = F.getSubprogram()) {  // 
    if (auto *File = SP->getFile()) {  // DIFile
      llvm::SmallString<128> Dir(File->getDirectory());
      llvm::SmallString<128> FileName(File->getFilename());
...
void ModuleSanitizerCoverageLTO::instrumentFunction(
  ...
  std::unordered_map<const BasicBlock *, std::vector<uint32_t>> succ_map;
  ...
  for (auto &BB : F) {
    ...
    if (!instrument_ctx)
      if (shouldInstrumentBlock(F, &BB, DT, PDT, Options)) {
        ...
        BlocksToInstrument.push_back(&BB);
        succ_map[&BB] = std::vector<uint32_t>(0);
  ...
  // 
  for (BasicBlock *InstrumentBlock : BlocksToInstrument) {

    const BasicBlock *start = InstrumentBlock;
    // 
    std::stack<const BasicBlock *> worker;  // ，
    ...
    while (!worker.empty()) {
      ...
      const BasicBlock *cur = worker.top();
      worker.pop();
      BasicBlock *nonConst_cur = const_cast<BasicBlock *>(cur);

      for (succ_iterator SI = succ_begin(nonConst_cur),
                         SE = succ_end(nonConst_cur);
           SI != SE; ++SI) {
        ...
        if (visited.find(succ) == visited.end()) {
          ...
          visited.insert(succ);
          worker.push(succ);
          if (succ_map.find(succ) != succ_map.end())  // key，
            succ_map[start].push_back(instrument_point_map[succ]);
  ...
  if (!outFile.is_open()) {
    ...
    outFile << "Function: " << llvm::demangle(F.getName().str())
            << "  File: " << getSourceFilePath(F)
            << "  Instrument_num:" << BlocksToInstrument.size() << std::endl;

    for (auto &pair : succ_map) {

      const BasicBlock      *bb = pair.first;
      std::vector<uint32_t> &succ = pair.second;

      outFile << "  InstPoint: " << instrument_point_map[bb]
              << "\tsucc_num: " << succ.size() << std::endl;
  ...
  if (!outFile1.is_open()) {
    ...
    for (auto &pair : succ_map) {
      ...
      outFile1 << instrument_point_map[bb] << ":\t";
      for (auto i : succ) {
  ...
  for (const auto &pair : succ_map) {

    const BasicBlock            *key = pair.first;
    const std::vector<uint32_t> &vec = pair.second;

    // （4 ）
    out_file.write(reinterpret_cast<const char *>(&(instrument_point_map[key])),
                   sizeof(int));
...
void ModuleSanitizerCoverageLTO::InjectCoverageAtBlock(Function   &F,
  ...
  if (Options.TracePCGuard) {
    ...
    // 
    instrument_point_map[&BB] = afl_global_id;
