...
enum SeedState { OTHER, CANDIDATE_TOP, CANDIDATE, EXECUTION };

// typedef struct InstrumentPointMap;
typedef struct instrument_point_map InstrumentPointMap;
struct queue_entry {

  enum SeedState state;

  u32    LB_count;                         /* select times for load balance */
  double WCSD;                       /* for master select load balance seed */
  double total_energy;
  // double contribute;
  // double gain;
  ...
  u8     is_top_k;                                               /*  */
  double min_dist_ES;                                         /*  */
  double fav_factor;                                         /* AFL++ */
  double priority;                                       /* AFL++ */

  double aggregation;               /* client: local aggregation degree     */
  u8     in_subdomain;              /* client: if in cur subdomain          */
  u8     not_sync;                    /* if not sync this seed(init seeds)  */
  double center_deviation;          /* client: distance to subdomain center */
  double potential;                 /* used in adjusting score              */
  double worth;                     /* worth of seed                        */
  u32    subdomain_id;              /* main: subdomain belong to            */
  u8     is_center;                 /* main: is subdomain center            */
  // double deviation;                      /* client: distance to the task */
  // double gain_score;                                  /* fuzz gain: seed */
...
typedef struct {
  ...
  struct queue_entry **items;                     /* seeds in sundomain     */
  u32                  size;                      /* seeds num in sundomain */
  struct queue_entry  *center;                    /* center of subdomain    */
  double               r;                         /* radius of subdomain    */
  u32                  id;                        /* id of sundomain        */
  u8                  *subdomain;       /* Instrumentation points in domain */
  double               overlap;                                         /*  */
  u8                   need_update_medoid;

  u32    contribute;         /* new seed saved by master, reset in new task */
  double sample_weight;                 /* sample weight for this subdomain */
  u32    LB_contribute;            /* contribute from LB, reset in new task */
  u32    total_contribute;                   /* total contribute in fuzzing */

  u32 cursor;                                   /* cursor for scan LB seeds */
  u32 max_LB;                                     /* max load balance times */
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  struct queue_entry **dist_queue_buf;       /* for asyn calculate distance */
  u32                  dist_queue_buf_len;         /* dist_queue_buf length */
  u32                  last_dist_queue_buf_len;    /* distance ready to use */

  struct queue_entry **select_queue_buf;               /* sort for priority */
  u32                  select_queued_items;      /* select_queue_buf length */

  double    **distance_matrix_cache;                      /* distance cache */
  u32         matrix_row_to_free;                     /* record for release */
  atomic_bool cal_dist_matrix;            /* is calculating distance matrix */

  pthread_mutex_t dist_cond_mutex;      /* lock(wait) for update distMatrix */
  pthread_cond_t  dist_cond;

  subdomain sd;                                   /* client: subdomain info */
  // u8 *subdomain_center;                           /* cur sundomain trace */
  // u32 subdomain_id;                            /* cur sundomain id in db */
  // double subdomain_r;                            /* cur sundomain radius */

  redisContext *redis_connect;
  s32           redis_db;                      /* which redis db used(0-15) */

  u64 last_divide_time;                              /* master: last divide */
  u64 last_fuzz_time;                            /* last time fuzz one seed */
  u64 last_check_overlap_time;               /* last time check if redivide */

  subdomain *subdomain_array;                                           /*  */
  u32        subdomain_num;                    /* length of subdomain_array */

  u8 not_do_cooperation,              /* if in cooperation, default open(0) */
      doing_cooperation,                            /* if doing cooperation */
      not_cal_deviation;          /* not calculat center deviation when add */

  u8 *global_seed_flag;                  /* if global seed already in local */
  u32 sync_cursor;

  u32 *freq_map;                                           /* edge cov freq */
  u64  freq_total;                                            /* total exec */

  // cooporation statistic
  u32 stat_collab_send;
  u32 total_coop_add;
  u32 total_LB_add;                       /* seed num add from load balance */

  // for normalize
  double max_reward;                 /* max reward within seeds in cur task */
  double min_reward;
  double max_potential;           /* max potential within seeds in cur task */
  double min_potential;

  InstrumentPointMap *inst_area;           /* area of each insttument point */

  struct queue_entry **candidate_set;                   /* candidate in ART */
  u32                  candidate_set_size;
  u32                  last_adjust_dist_len;

  double max_factor;                            /* max factor for normalize */
  double min_factor;

  double max_dist_to_ES;                      /* max distance for normalize */
  double min_dist_to_ES;

  // double max_gain;                                     /* max gain for seed
  // */ double min_gain;

  double max_total_energy;
  double min_total_energy;

  u64 now_time;                                         /*  current time */
  u64 last_priority_time;                                      /*  */
  u64 last_LB_sort_time;                                      /*  */
  u64 last_LB_time;                                          /*  */

  double total_dist_time;                                     /*  */

  GHashTable *Iname_Sid_map;                            /* subdomain */

  u8 *trace_mini_tmp_buf;                  /* a fixed buffer for trace_mini */
  u8  fuzz_LB_seed;                  /* force to fuzz seed for load balance */
  ...
  u32    *send_alias_table;                                             /*  */

  double *recv_probability;                                         /*      */
  u32    *recv_alias_table;                                              /* */

  u32 LB_seed_id;                                       /* record to delete */

  FILE *exp_fd;                                 /* file for experiment data */

  u64 peer_sync_count;                            /* sync from master times */
  u64 master_sync_count;                     /* sync from cooperation times */
...
void afl_redis_init(afl_state_t *afl, const char *hostname, int port, int db);
