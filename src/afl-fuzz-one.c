...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  // u32 queued_items_before = afl->queued_items;
  ...
  if (likely(afl->pending_favored)) {
    ...
    if ((afl->queue_cur->fuzz_level || !afl->queue_cur->favored) &&
      ...
      if (likely(!afl->fuzz_LB_seed)) return 1;
  ...
  //     if ((afl->queue_cur->fuzz_level ||
  //          (!afl->queue_cur->favored || !afl->queue_cur->in_subdomain)) &&
  //         likely(rand_below(afl, 100) < SKIP_TO_NEW_PROB)) {

  //       return 1;
  //     }

  //   } else if (!afl->non_instrumented_mode &&
  //              (!afl->queue_cur->favored || !afl->queue_cur->in_subdomain) &&
  ...
  if (likely(afl->not_on_tty)) {
    ...
    ACTF(
        ...
        "mode=%s, perf_score=%0.0f, weight=%0.0f, favorite=%u(pending %u), "
        "was_fuzzed=%u, exec_us=%llu, hits=%u, map=%u, ascii=%u, run_time=%s, "
        "fuzz_level=%u, total_energy=%.1f)...",
        ...
        afl->queue_cur->was_fuzzed, afl->queue_cur->exec_us,
        likely(afl->n_fuzz) ? afl->n_fuzz[afl->queue_cur->n_fuzz_entry] : 0,
        afl->queue_cur->bitmap_size, afl->queue_cur->is_ascii, time_tmp,
        afl->queue_cur->fuzz_level, afl->queue_cur->total_energy);
    fflush(stdout);
  ...
  if (likely(!afl->old_seed_selection)) {
    ...
    if (afl->is_secondary_node && afl->sd.center && afl->max_total_energy &&
        !afl->fuzz_LB_seed) {  // 
      ...
      double progress_to_tx_t = pow(((double)t) / ((double)100 * 60.0), 0.5);
      // double progress_to_tx = ((double)t) / ((double)100 * 60.0);  // 100min
      // ，d；d
      double progress_to_tx_e = 1 - (q->total_energy / afl->max_total_energy);
      double progress_to_tx = progress_to_tx_t;

      double T;
      T = 1.0 / pow(20.0, progress_to_tx);

      double power_factor = 1.0;
      double normalized_d = 0.0;
      double normalized_f = 0.0;

      if (q->min_dist_ES > 0 &&
          afl->max_dist_to_ES != DBL_MAX) {  // 

        // when "max_distance == min_distance", we set the
        // normalized_d to 0 so that we can sufficiently
        // explore those testcases whose distance >= 0.

        if (afl->max_dist_to_ES != afl->min_dist_to_ES) {

          // 
          normalized_d = (q->min_dist_ES - afl->min_dist_to_ES) /
        ...
        if (afl->max_factor != afl->min_factor) {
          ...
          normalized_f = (q->fav_factor - afl->min_factor) /
                         (afl->max_factor - afl->min_factor);

        }

        double normalized_p = 0.7 * normalized_d + 0.3 * (1.0 - normalized_f);

        if (normalized_p >= 0) {

          double p = normalized_p * progress_to_tx_e * (1.0 - T) + 0.5 * T;
      ...
      if (q->perf_score < 1.0) q->perf_score = 100;  // 0
      q->perf_score *= power_factor;

      /* Make sure that we don't go over limit. */

      if (q->perf_score > afl->havoc_max_mult * 100) {

        q->perf_score = afl->havoc_max_mult * 100;

      }

      if (power_factor != 1.0) {

        DCPF_DEBUG("t:%.4f, TE:%.4f, n_d:%.4f, n_f:%.4f", progress_to_tx_t,
                   progress_to_tx_e, normalized_d, normalized_f);
        DCPF_DEBUG(
            "[Time %llu] "
            "q->min_dist_ES: %4lf, "
            "min_dist_to_ES: %4lf, "
            ...
            "T: %4.3lf, "
            "power_factor: %4.3lf, "
            "adjusted perf_score: %u",
            t, q->min_dist_ES, afl->min_dist_to_ES, afl->max_dist_to_ES,
            q->fav_factor, afl->min_factor, afl->max_factor, T, power_factor,
            (u32)q->perf_score);
    ...
    //   double progress_to_tx = pow(((double)t) / ((double)100 * 60.0), 0.5);
    //   // double progress_to_tx = ((double)t) / ((double)100 * 60.0);  //
    //   100min
    //   // d，d
    //   progress_to_tx *= 1 - (q->total_energy / afl->max_total_energy);
    ...
    //       double p = normalized_d * T + 0.5 * (1.0 - T);
    //       double max_factor = 16.0;
    //       power_factor = pow(2.0, 2.0 * (double)log2(max_factor) * (p -
    //       0.5));
    ...
    //     DCPF_DEBUG("t:%.4f, f:%.4f, d:%.4f",
    //                pow(((double)t) / ((double)100 * 60.0), 0.5),
    //                q->total_energy / afl->max_total_energy,
    //                (q->min_dist_ES - afl->min_dist_to_ES) /
    //                    (afl->max_dist_to_ES - afl->min_dist_to_ES));
    //     DCPF_DEBUG(
    //         "[Time %llu] "
    //         "q->min_dist_ES: %4lf, "
    ...
    //         t, q->min_dist_ES, afl->max_dist_to_ES, afl->min_dist_to_ES, T,
    //         power_factor, (u32)q->perf_score);
    //   }
    // }
    ...
    /*   // if (afl->is_secondary_node) {
      ...
      //     double n_gain =
      //         (q->gain - afl->min_gain) / (afl->max_gain - afl->min_gain);

      //     double n_total_energy = (q->total_energy - afl->min_total_energy) /
      //                             (afl->max_total_energy -
      afl->min_total_energy);

      //     // ，
      //     // ， 0 1 
      //     if (n_total_energy > 0.9 && n_gain < 0.5) {

      //       q->perf_score /= 20;
      //       DCPF_DEBUG("cut score for seed#%u", q->id);
      //     }
      //   }
      // }
     */
    ...
    // if (q->is_top_k) q->perf_score *= 5;  //  50% ， 5
    // 
    ...
    //   double power_factor = 1.0;
    //   if (q->total_energy > 0) {  // fuzz

    //     // when "max_distance == min_distance", we set the
    //     // normalized_d to 0 so that we can sufficiently
    //     // explore those testcases whose distance >= 0.
    //     double normalized_e = 0;

    //     if (afl->max_total_energy != afl->min_total_energy) {

    //       normalized_e = (q->total_energy - afl->min_total_energy) /
    //                      (afl->max_total_energy - afl->min_total_energy);
    //     }

    //     if (normalized_e >= 0) {

    //       double p = (1.0 - normalized_e) * (1.0 - T) + 0.5 * T;
    //       double max_factor = 16.0;
    //       power_factor = pow(2.0, 2.0 * (double)log2(max_factor) * (p -
    //       0.5));
    ...
    //         "[Time %llu] "
    //         "q->total_energy: %4lf, "
    //         "max_total_energy: %4lf, "
    //         "min_total_energy: %4lf, "
    //         "T: %4.3lf, "
    //         "power_factor: %4.3lf, "
    //         "adjusted perf_score: %4d",
    //         t, q->total_energy, afl->max_total_energy, afl->min_total_energy,
    //         T, power_factor, (u32)q->perf_score);
    // }

    orig_perf = perf_score = q->perf_score;
...
abandon_entry:
  ...
  afl->queue_cur->total_energy += perf_score;
  // afl->queue_cur->contribute += (afl->queued_items - queued_items_before);
