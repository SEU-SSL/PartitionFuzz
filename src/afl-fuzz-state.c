...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->cal_dist_matrix = ATOMIC_VAR_INIT(false);
  pthread_cond_init(&afl->dist_cond, NULL);

  if (pthread_mutex_init(&afl->dist_cond_mutex, NULL) != 0)
    FATAL("dist_cond_mutex initialization failed");
...
/* Initializes redis connect. */

void afl_redis_init(afl_state_t *afl, const char *hostname, int port, int db) {

  DCPF_LOG("create redis connection pool...");

  afl->redis_connect = redisConnect(hostname, port);
  if (afl->redis_connect == NULL || afl->redis_connect->err) {

    if (afl->redis_connect) {

      WARNF("Connection error, check redis: %s", afl->redis_connect->errstr);
      redisFree(afl->redis_connect);

    } else {

      WARNF("Error: Cannot allocate Redis context\n");

    }

  }

  redisReply *reply = redisCommand(afl->redis_connect, "SELECT %d", db);
  if (reply == NULL) {

    WARNF("Error selecting database: %s\n", afl->redis_connect->errstr);
    redisFree(afl->redis_connect);
    FATAL("Failed to create Redis pool");

  }

  if (reply->type == REDIS_REPLY_ERROR) {

    WARNF("Redis error: %s", reply->str);
  ...
  if (afl->is_main_node) {
    ...
    redisReply *reply = redisCommand(afl->redis_connect, "FLUSHDB");
    if (reply == NULL) {
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  pthread_cond_destroy(&afl->dist_cond);
  pthread_mutex_destroy(&afl->dist_cond_mutex);

  if (afl->dist_queue_buf) afl_free(afl->dist_queue_buf);
  if (afl->select_queue_buf) afl_free(afl->select_queue_buf);

  for (u32 i = 0; i < afl->matrix_row_to_free; i++)
    afl_free(afl->distance_matrix_cache[i]);
  afl_free(afl->distance_matrix_cache);

  if (afl->sd.center) {

    ck_free(afl->sd.center->trace_mini);
    ck_free(afl->sd.center);

  }

  for (u32 k = 0; k < afl->subdomain_num; k++) {

    // free in destroy_queue
    // ck_free(afl->subdomain_array[k].center->trace_mini);
    ck_free(afl->subdomain_array[k].center);

  }

  afl_free(afl->subdomain_array);
  afl_free(afl->global_seed_flag);
  ck_free(afl->store_trace);
  ck_free(afl->freq_map);
#ifdef PATH_SIZE
  ck_free(afl->path);
#endif

  // if (afl->inst_area)
  //   destroy_instrument_map(afl->inst_area, afl->fsrv.map_size);

  g_hash_table_destroy(afl->Iname_Sid_map);
  ck_free(afl->trace_mini_tmp_buf);

  afl_free(afl->send_alias_table);
  afl_free(afl->recv_alias_table);
  afl_free(afl->send_probability);
  afl_free(afl->recv_probability);
...
void afl_states_stop(void) {
  ...
  LIST_FOREACH(&afl_states, afl_state_t, {
    ...
    pthread_cond_broadcast(&el->dist_cond);
