...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  double center_dist = 0.0;                      /* distance to self center */
  u8    *temp_trace_mini = NULL;
  u32    subdomain_assign = UINT32_MAX;
  u8     calculated = 0;  // trace_mini
  ...
  /*  */
  // update_edge_freq(afl, afl->freq_map);
  ...
  if (likely(fault == afl->crash_mode)) {
     ...
     *
     * ：
     * 1. afl->not_do_cooperation 
     * 2. afl
     */

    /* seed cooperation send */

    if (afl->is_secondary_node && afl->sd.center && !afl->not_do_cooperation) {

      /* fuzz */

      /* if insteresting and the distance to current medoids > current
         cluster r, send to nearest medious */

      calculated = 1;

      if (!temp_trace_mini)
        temp_trace_mini = (u8 *)ck_alloc(afl->fsrv.map_size >> 3);
      minimize_bits(afl, temp_trace_mini, afl->fsrv.trace_bits);

      center_dist = get_trace_mini_distance(afl, afl->sd.center->trace_mini,
                                            temp_trace_mini);

      double     temp_min = DBL_MAX;
      subdomain *send_to = NULL;  // instance to send

      if (center_dist > afl->sd.r) {

        temp_min = center_dist;  // 

        // select one worker to send
        for (u32 k = 0; k < afl->subdomain_num; k++) {

          if (k == afl->sd.id) continue;  // 

          double dist_to_other = 0.0;

          dist_to_other = get_trace_mini_distance(
              afl, afl->subdomain_array[k].center->trace_mini, temp_trace_mini);

          if (dist_to_other < temp_min) {

            temp_min = dist_to_other;
            send_to = &afl->subdomain_array[k];
        ...
        if (!send_to) {
          ...
          if (afl->fuzz_LB_seed) {
            ...
            redisReply *reply =
                redisCommand(afl->redis_connect, "RPUSH %s_LB_report %b",
                             afl->sync_id, mem, len);
            // DCPF_DEBUG("RPUSH %s_LB_report", afl->sync_id);
          ...
          // redis
          redisReply *reply =
              redisCommand(afl->redis_connect, "RPUSH coop_queue_%u %b",
                           send_to->id, mem, len);
          ...
          ++afl->stat_collab_send;

          // ck_free(temp_trace_mini);
          // return 0; // 
    ...
    // if (afl->is_main_node && afl->subdomain_array &&
    // !afl->not_do_cooperation) {
    ...
    if (afl->is_main_node && afl->subdomain_array) {
      ...
      for (u32 k = 0; k < afl->subdomain_num; k++) {
        ...
        double dist_to_subdomain = get_trace_mini_distance(
            afl, afl->subdomain_array[k].center->trace_mini, temp_trace_mini);

        if (dist_to_subdomain < temp_min) {

          temp_min = dist_to_subdomain;
          subdomain_assign = k;

        }

        // item，、
        u32 n = ++afl->subdomain_array[k].size;
        afl->subdomain_array[k].items =
            afl_realloc((void **)&afl->subdomain_array[k].items,
                        sizeof(struct queue_entry *) * n);
        afl->subdomain_array[k].items[n - 1] = afl->queue_top;

      }

      // subdomain_array
      if (temp_min > afl->subdomain_array[subdomain_assign].r)
        afl->subdomain_array[subdomain_assign].r = temp_min;
  ...
  save_to_queue:
    ...
    if (!afl->fuzz_LB_seed) {  // not persistance
    ...
    if (afl->fuzz_LB_seed) {  // temp in memeory
      afl->queue_top->testcase_buf = (u8 *)malloc(len);
      if (unlikely(!afl->queue_top->testcase_buf))
        PFATAL("Unable to malloc LB seed with len %u", len);

      memcpy(afl->queue_top->testcase_buf, mem, len);
    ...
    if (afl->is_main_node) {
      ...
      // worker
      if (afl->syncing_party) { afl->queue_top->not_sync = 1; }

      afl->queue_top->subdomain_id = subdomain_assign;
      save_one_seed_to_db(afl->redis_connect, afl->queue_top, mem);
    ...
    if (calculated) {
      ...
      afl->queue_top->trace_mini = temp_trace_mini;

    } else {

      afl->queue_top->trace_mini = (u8 *)ck_alloc(afl->fsrv.map_size >> 3);
      minimize_bits(afl, afl->queue_top->trace_mini, afl->fsrv.trace_bits);

    }

    /* client: */

    if (afl->is_secondary_node && afl->sd.center && !afl->not_cal_deviation) {

      if (calculated)
        afl->queue_top->center_deviation = center_dist;
      else
        afl->queue_top->center_deviation = get_trace_mini_distance(
            afl, afl->sd.center->trace_mini, afl->queue_top->trace_mini);

      // client ，，
      if (afl->queue_top->center_deviation <= afl->sd.r)
        afl->queue_top->in_subdomain = 1;
    ...
    if (likely(afl->q_testcase_max_cache_size && !afl->fuzz_LB_seed)) {
