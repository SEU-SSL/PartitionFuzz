...
void free_one_seed(struct queue_entry *q) {
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
    ...
    switch (opt) {
      ...
      case 'H': {  // select redis db

        if (afl->redis_db != -1)
          FATAL("Multiple -H(--db) options not supported");

        if (sscanf(optarg, "%d", &afl->redis_db) < 0) {

          FATAL("Bad syntax used for -H(--db)");
      ...
      case 'M': {                                           /* main sync ID */
        ...
        // afl->old_seed_selection = 1;  // force old queue walking seed
        // selection
        afl->disable_trim = 1;
  ...
  afl_redis_init(afl, "127.0.0.1", 6379, afl->redis_db);
  // 
  #ifdef PATH_SIZE
  afl->path = ck_alloc(PATH_SIZE * sizeof(u8));
  #endif
  ...
  // open the file for the experiment data
  u8 *filename = alloc_printf("%s/exp_data.txt", afl->out_dir);

  afl->exp_fd = fopen((char *)filename, "a");
  if (afl->exp_fd == NULL) {
  ...
  afl->store_trace = ck_alloc(map_size);
  if (!afl->store_trace) FATAL("ck_alloc error");

  afl->freq_map = ck_alloc(afl->fsrv.map_size * sizeof(u32));
  if (!afl->freq_map) FATAL("ck_alloc error");

  afl->trace_mini_tmp_buf = (u8 *)ck_alloc(afl->fsrv.map_size >> 3);
  if (!afl->trace_mini_tmp_buf) FATAL("ck_alloc error");

  // char *file_path = getenv("INSTRUMENT_MAP_PATH");
  // if (!file_path) FATAL("INSTRUMENT_MAP not set!");
  // DCPF_LOG("，from %s...", file_path);
  // afl->inst_area = load_instrument_map(file_path, afl->fsrv.map_size);
  ...
  if (afl->is_main_node || afl->is_secondary_node) {
    ...
    pthread_t tid;
    pthread_create(&tid, NULL, update_distance_matrix_thread, (void *)afl);
    bind_all_cpu(afl->cpu_to_bind, tid);
    pthread_detach(tid);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    // don't mark LB seed as redundant
    if (!afl->fuzz_LB_seed) cull_queue(afl);
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      if (unlikely((afl->last_sync_cycle < afl->queue_cycle ||
        ...
        if (afl->is_main_node)
          sync_fuzzers(afl);
        else if (afl->sd.center) {

          sync_subdomain_seeds_from_db(afl, afl->sd.id);
    ...
    do {
      ...
      if (likely(!afl->old_seed_selection && !afl->fuzz_LB_seed)) {

        if (likely(afl->pending_favored && afl->smallest_favored >= 0)) {

          afl->current_entry = afl->smallest_favored;

          u32 last_dist_queue_buf_len = afl->last_dist_queue_buf_len;

          if (likely(afl->pending_favored > 1)) {

            //  favored && not_fuzzed && 
            // DCPF_LOG("dist seed num: %u/%u", last_dist_queue_buf_len,
            //          afl->queued_items);

            afl->max_dist_to_ES = -DBL_MAX;
            afl->min_dist_to_ES = DBL_MAX;
            afl->max_factor = -DBL_MAX;
            afl->min_factor = DBL_MAX;

            u32 candidate_num = 0;
            u32 execution_num = 0;

            // ，
            for (u32 i = 0; i < last_dist_queue_buf_len; i++) {
              ...
              for (u32 j = 0; j < last_dist_queue_buf_len; j++) {

                if (!afl->queue_buf[j]->was_fuzzed ||
                    afl->queue_buf[j]->disabled)
                  continue;

                execution_num++;

                double dist = get_distance_from_matrix(afl, i, j);
              ...
              if (min_dist == DBL_MAX) {  // 
                ...
                afl->queue_buf[i]->min_dist_ES = min_dist;

                afl->max_dist_to_ES = MAX(min_dist, afl->max_dist_to_ES);
                afl->min_dist_to_ES = MIN(min_dist, afl->min_dist_to_ES);

              }

              double factor =
                  (double)afl->queue_buf[i]->exec_us * afl->queue_buf[i]->len;
              afl->queue_buf[i]->fav_factor = factor;

              afl->min_factor = MIN(afl->min_factor, factor);
              afl->max_factor = MAX(afl->max_factor, factor);

            }

            if (candidate_num >= 2 && execution_num) {

              DCPF_DEBUG("min_dist_to_ES=%f, max_dist_to_ES=%f",
                         afl->min_dist_to_ES, afl->max_dist_to_ES);
              ...
              for (u32 i = 0; i < last_dist_queue_buf_len; i++) {
                ...
                double n_factor;
                if (afl->max_factor != afl->min_factor &&
                    afl->min_factor != -DBL_MAX) {

                  n_factor = (q->fav_factor - afl->min_factor) /
                             (afl->max_factor - afl->min_factor);
                ...
                double n_dist_to_ES;
                if (afl->max_dist_to_ES != afl->min_dist_to_ES &&
                    afl->min_dist_to_ES != -DBL_MAX) {

                  n_dist_to_ES = (q->min_dist_ES - afl->min_dist_to_ES) /
                                 (afl->max_dist_to_ES - afl->min_dist_to_ES);
              ...
              DCPF_DEBUG("    IF select one seed, max_score=%f", max_score);
      ...
      if (!afl->pending_favored && afl->queue_cur->use_det)
    ...
    if (afl->fuzz_LB_seed) {
      ...
      afl->fuzz_LB_seed = 0;  // end LB fuzzing

      //  afl->LB_seed_id 。
      // LB，

      // ，
      if (afl->LB_seed_id + 1 == afl->queued_items) {

        if (likely(afl->queue_buf[afl->LB_seed_id]->len > 4)) {

          --afl->ready_for_splicing_count;

        }

        struct queue_entry *q = afl->queue_buf[afl->LB_seed_id];
        if (q->tc_ref) {
        ...
        free_one_seed(afl->queue_buf[afl->LB_seed_id]);
        afl->queue_buf[afl->LB_seed_id] = NULL;

        DCPF_LOG("only  LB  %u", afl->LB_seed_id);
        ...
        // ，
        u8  tmp[10];
        u32 cur = afl->LB_seed_id + 1;
        u32 count = 0;
        while (cur < afl->queued_items) {

          struct queue_entry *q = afl->queue_buf[cur];
          --q->id;

          // 
          char *id_ptr = strstr(q->fname, "id:");  //  id:
          if (!id_ptr) FATAL("id_ptr is NULL");
          // id: 6，id_ptr + 3 
          snprintf(tmp, sizeof(tmp), "%06u", q->id);  // 6 digits + \0
          memcpy(id_ptr + 3, tmp, 6);  // ， \0

          // 
          afl->queue_buf[cur - 1] = q;

          // 
          // DCPF_LOG(" %s", q->fname);
          ++count;
          s32 fd = permissive_create(afl, q->fname);
          if (likely(fd >= 0)) {

            ck_write(fd, q->testcase_buf, q->len, q->fname);
            close(fd);
      ...
      afl->queue_top = afl->queue_buf[afl->queued_items - 1];
    ...
    if (afl->is_secondary_node &&
        !if_coopqueue_empty(afl->redis_connect, afl->sd.id)) {
    ...
    if (!atomic_load(&afl->cal_dist_matrix) &&
        afl->queued_items > afl->dist_queue_buf_len) {

      inform_update_distance_matrix(afl);
    ...
    if (afl->is_secondary_node) {
      ...
      if (if_need_update_subdomain(afl->redis_connect, afl->sync_id)) {
        ...
        // 
        update_subdomain(afl);

        // 
        change_subdomain_state(afl->redis_connect, afl->sync_id, 0);

        // 
        DCPF_LOG("sync subdomains info from db");
        get_all_subdomain_from_db(afl->redis_connect, &afl->subdomain_array,
                                  &afl->subdomain_num);
        ...
        if (!afl->old_seed_selection) {
          ...
          // ，。！
          afl->current_entry = afl->queued_items + 1;
       ...
       *    4. fuzz，（id），、
       * 
       *    1. disable persistance, temp seed in memory
       *    2. disable energy adjust
       *    3. disable skip not favored seed
       */

      if (get_LBqueue_size(afl->redis_connect, afl->sd.id) > 0) {

        afl->fuzz_LB_seed = 1;  // need disable for normal fuzzing
        cull_queue(afl);        // don't mark LB seed as redundant

        /* pop one to fuzz next */
        afl->LB_seed_id = do_load_balance(afl);

        if (afl->LB_seed_id != UINT32_MAX) {

          afl->current_entry = afl->LB_seed_id;
          afl->queue_cur = afl->queue_buf[afl->current_entry];
          afl->queue_cur->trim_done = 1;  // disable trim

          DCPF_LOG(", fuzz seed#%u, total fuzz %u LB seeds",
                   afl->LB_seed_id, afl->total_LB_add);
...
stop_fuzzing:
  ...
  for (u32 i = 0; i < afl->subdomain_num; i++) {

    fprintf(
        afl->exp_fd,
        "subdomain %u contribute %u + %u in cur_task, total contribute %u \n",
        i, afl->subdomain_array[i].contribute,
        afl->subdomain_array[i].LB_contribute,
        afl->subdomain_array[i].total_contribute);

    fprintf(afl->exp_fd, "subdomain %u send %u, recv %u \n", i,
            afl->subdomain_array[i].LBsend, afl->subdomain_array[i].LBrecv);

    fprintf(afl->exp_fd,
            "sync from master %llu times, sync from cooperation %llu times, "
            "total %llu \n",
            afl->master_sync_count, afl->peer_sync_count,
            afl->master_sync_count + afl->peer_sync_count);
  ...
  if (afl->afl_env.afl_final_sync) {
    ...
    if (afl->is_main_node) sync_fuzzers(afl);
    write_bitmap(afl);
    SAYF(cYEL "[!] " cRST "Done!\n\n");
