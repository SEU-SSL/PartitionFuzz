...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  if (afl->fuzz_LB_seed) afl->stage_max = 2;
...
void sync_fuzzers(afl_state_t *afl) {
  ...
  u_simplestring_time_diff(time_tmp, afl->prev_run_time + get_cur_time(),
                           afl->start_time);
  DCPF_DEBUG("MASTER sync start at %s", time_tmp);
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    // subdimainid
    u32      old_import = afl->queued_imported;
    int      subdomain_id = -1;
    gpointer value = g_hash_table_lookup(afl->Iname_Sid_map, sd_ent->d_name);
    ...
    for (o = m; o < n; o++) {
      ...
      // 
      if (strstr(namelist[o]->d_name, ",orig:") != NULL) continue;
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        afl->queued_imported +=
            save_if_interesting(afl, mem, new_len, fault, 0);
        show_stats(afl);
        afl->syncing_party = 0;
  ...
  close_sync:
    ...
    if (subdomain_id >= 0) {
      ...
      u32 new_contri = afl->queued_imported - old_import;
      afl->subdomain_array[subdomain_id].contribute += new_contri;
      afl->subdomain_array[subdomain_id].total_contribute += new_contri;

      fprintf(
          afl->exp_fd,
          "subdomain %u contribute %u + %u in cur_task, total contribute %u \n",
          subdomain_id, afl->subdomain_array[subdomain_id].contribute,
          afl->subdomain_array[subdomain_id].LB_contribute,
          afl->subdomain_array[subdomain_id].total_contribute);

      afl->min_contribute = MIN(afl->min_contribute,
                                afl->subdomain_array[subdomain_id].contribute);
      afl->max_contribute = MAX(afl->max_contribute,
                                afl->subdomain_array[subdomain_id].contribute);
  ...
  u_simplestring_time_diff(time_tmp, afl->prev_run_time + afl->last_sync_time,
                           afl->start_time);
  DCPF_DEBUG("MASTER sync end at %s", time_tmp);
