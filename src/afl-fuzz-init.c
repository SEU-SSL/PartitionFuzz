...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    /* trace_mini */
    if (!q->trace_mini) q->trace_mini = (u8 *)ck_alloc(afl->fsrv.map_size >> 3);
    ...
    if (afl->is_main_node) save_one_seed_to_db(afl->redis_connect, q, use_mem);
