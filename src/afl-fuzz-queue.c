...
void create_alias_table(afl_state_t *afl) {
  ...
  if (afl->is_secondary_node &&
      afl->now_time - afl->last_priority_time > 600000 &&
      last_dist_queue_buf_len > afl->last_adjust_dist_len) {

    u32 not_fuzzed_num = 0, fuzzed_num = 0;
    u32 disabled_num = 0;
    u32 favored_num = 0;
    ...
    for (u32 i = 0; i < last_dist_queue_buf_len; i++) {
      ...
      if ((q->in_subdomain && !q->was_fuzzed) || q->favored)
        afl->candidate_set_size++;
    ...
    disabled_num = last_dist_queue_buf_len - fuzzed_num - not_fuzzed_num;
    printf("    (size=%u), dist seed num: %u/%u (%u disabled)\n",
           afl->candidate_set_size, last_dist_queue_buf_len, afl->queued_items,
           disabled_num);

    /* ARTfavored */

    // 1. ART（）
    // not_fuzzed, fuzzed, FSCSART
    // fuzz,,
    // FSCSART(,)
    ...
    // afl->candidate_set_size = not_fuzzed_num + favored_num;
    afl->candidate_set =
    ...
    assert(candidate_set_offset == afl->candidate_set_size);
    ...
    for (u32 i = 0; i < afl->candidate_set_size; i++) {
      ...
      for (u32 j = 0; j < last_dist_queue_buf_len; j++) {
        ...
        // （was_fuzzed & in_subdomain）
        if (!afl->queue_buf[j]->was_fuzzed || afl->queue_buf[j]->disabled ||
            !afl->queue_buf[j]->in_subdomain)
          continue;

        // ：fuzz && disabled && 
        double dist =
            get_distance_from_matrix(afl, afl->candidate_set[i]->id, j);

        if (dist < min_d && dist > 0) { min_d = dist; }

      }

      // assert(min_d != DBL_MAX);
    ...
    //   afl->candidate_set[i]->min_dist_ES =
    //       (afl->candidate_set[i]->min_dist_ES - min_min_dist_ES) /
    //       (max_min_dist_ES - min_min_dist_ES);
    // }
    ...
    quickSort(afl->candidate_set, 0, afl->candidate_set_size - 1);

    //  50% ,  50%, 30% 
    u32 half = afl->candidate_set_size >> 1;
    for (u32 i = 0; i < afl->candidate_set_size; i++) {

      if (i < half) {

        // DCPF_DEBUG(
        //     "set seed#%u to top_k, favored=%u, min_dist_ES=%.2f, "
        //     "fav_factor=%.2f, exec_us=%llu, len=%u, map_size=%u",
        //     afl->candidate_set[i]->id, afl->candidate_set[i]->favored,
        //     afl->candidate_set[i]->min_dist_ES,
        //     afl->candidate_set[i]->fav_factor,
        //     afl->candidate_set[i]->exec_us, afl->candidate_set[i]->len,
        //     afl->candidate_set[i]->bitmap_size);
        afl->candidate_set[i]->is_top_k = 1;
        afl->candidate_set[i]->state = CANDIDATE_TOP;

      } else if (!afl->candidate_set[i]->passed_det &&

                 rand_below(afl, 100) > 70) {  // 30%

        afl->candidate_set[i]->use_det = 1;

      }

    }

    afl->last_adjust_dist_len = last_dist_queue_buf_len;
    afl->last_priority_time = get_cur_time();
  ...
  // afl->max_gain = -DBL_MAX;
  // afl->min_gain = DBL_MAX;
  afl->max_total_energy = -DBL_MAX;
  afl->min_total_energy = DBL_MAX;
  ...
  if (likely(afl->schedule < RARE)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (likely(!q->disabled)) {
        ...
        // , 0 => contribute
        if (q->total_energy == 0.0) {  // ，

          // q->gain = 0.0;

        } else {  // 0

          // q->gain = (q->contribute + 1) / q->total_energy;
          // afl->min_gain = MIN(q->gain, afl->min_gain);
          // afl->max_gain = MAX(q->gain, afl->max_gain);
          afl->max_total_energy = MAX(afl->max_total_energy, q->total_energy);
          afl->min_total_energy = MIN(afl->min_total_energy, q->total_energy);
      ...
      if (likely(!q->disabled)) {
        ...
        {  // inline does result in a compile error with LTO, weird
          ...
          if (afl->is_secondary_node) {
            ...
            if (q->state == CANDIDATE_TOP) {

              weight *= 1.5;

            } else if (q->state == CANDIDATE) {

              weight *= 1.3;

            } else if (q->state == EXECUTION) {
